...
export default function Workbook() {
  ...
  return <>
    ...
    <section className="section" id="workbook-preview"><div className="container"><SectionHeader eyebrow="Workbook previews" title="Reflection becomes <em>direction.</em>" description="Select any page to enlarge it and see how reflection becomes a practical decision, plan, or action." align="center"/><PreviewGallery className="page-preview-four" items={[{src:'/assets/workbook-cover.png',alt:'Workbook cover'},{src:'/assets/workbook-contents.png',alt:'Workbook table of contents'},{src:'/assets/workbook-how-to.png',alt:'How to use the workbook'},{src:'/assets/workbook-assessment.png',alt:'Confidence assessment worksheet'}]} /></div></section>
    ...
    <FounderSection />
    {testimonials.some((testimonial) => !testimonial.placeholder) && <section className="section testimonial-section"><div className="container"><SectionHeader eyebrow="Transformation stories" title="Results that become <em>real life.</em>" align="center"/><div className="testimonial-grid">{testimonials.filter((testimonial) => !testimonial.placeholder).map((t,i)=><TestimonialCard testimonial={t} key={i}/>)}</div></div></section>}
