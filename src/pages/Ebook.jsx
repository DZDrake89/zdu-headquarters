...
export default function Ebook() {
  ...
  return <>
    ...
    <section className="section topic-section"><div className="container"><SectionHeader eyebrow="What you will learn" title="Understand the pattern.<br/><em>Change the system.</em>" description="Move from confusion and self-judgment to clarity, self-trust, and repeatable action."/><div className="topic-grid">{topics.map(([title,Icon],i)=><article key={title}><span>{String(i+1).padStart(2,'0')}</span><Icon/><h3>{title}</h3></article>)}</div></div></section>
    <section className="section preview-gallery" id="inside"><div className="container"><SectionHeader eyebrow="Look inside" title="Clear. Practical. <em>Actionable.</em>" description="Select any page to enlarge it and explore the guide before you buy." align="center" light/><PreviewGallery items={[{src:'/assets/ebook-contents.png',alt:'Ebook table of contents'},{src:'/assets/ebook-framework.png',alt:'Calm Confidence Framework page'},{src:'/assets/ebook-challenge.png',alt:'30-day confidence challenge page'}]} /></div></section>
